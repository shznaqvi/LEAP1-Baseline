package edu.aku.hassannaqvi.leap1_baseline.syncclasses;
 ...
 */
public class SyncNutrition extends AsyncTask<Void, Void, String> {

    private static final String TAG = "SyncNutrition";
    private Context mContext;
    private ProgressDialog pd;

    public SyncNutrition(Context context) {
        mContext = context;
    }
    ...
    protected String doInBackground(Void... params) {
        ...
        try {
            String url = AppMain.PROJECT_URI + NutritionContract.NutritionTable._URL;
...
/*    @Override
        ...
        try {
            ...
            for (int i = 0; i < json.length(); i++) {
                ...
                if (jsonObject.getString("status").equals("1") && jsonObject.getString("error").equals("0")) {
                    db.updateNutrition(jsonObject.getString("id"));
                    sSynced++;
                } else {
            ...
            Toast.makeText(mContext, sSynced + " Nutrition synced." + String.valueOf(json.length() - sSynced) + " Errors: " + sSyncedError, Toast.LENGTH_SHORT).show();

            pd.setMessage(sSynced + " Nutrition synced." + String.valueOf(json.length() - sSynced) + " Errors: " + sSyncedError);
            pd.setTitle("Done uploading Forms data");
            pd.show();
        } catch (JSONException e) {
            e.printStackTrace();
            ...
            pd.setMessage(result);
            pd.setTitle("Nutrition Sync Failed");
            pd.show();
        }
    }
    ...
    private String downloadUrl(String myurl) throws IOException {
        ...
        //  int len = 500;
        DatabaseHelper db = new DatabaseHelper(mContext);
        Collection<NutritionContract> nc = db.getUnsyncedNutrition();
        Log.d(TAG, String.valueOf(nc.size()));
        if (nc.size() > 0) {
            try {
                ...
                try {
                    ...
                    for (NutritionContract nt : nc) {

                        jsonSync.put(nt.toJSONObject());
